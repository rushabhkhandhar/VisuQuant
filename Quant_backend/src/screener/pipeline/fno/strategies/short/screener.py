...
class FnoShortScreener:
    ...
    def screen(self, target_date: date, daily_data: Dict[str, pd.DataFrame], industry_map: Dict[str, str]) -> List[str]:
        """
        Returns the list of weakest F&O stocks within the weakest sectors on the target_date.
        """
        ...
        # Regime Filter: If > 50% of F&O stocks are above their 50 SMA, we are in a Bull Market. Block all shorts.
        if total_valid_stocks > 0:
            breadth_pct = stocks_above_50sma / total_valid_stocks
            if breadth_pct > 0.50:
                logger.debug(f"Bull Market Regime detected (Breadth {breadth_pct:.1%}). Blocking short trades.")
                return []
        ...
        # Sort sectors by weakest first
        weakest_sectors = sorted(sector_momentum.keys(), key=lambda k: sector_momentum[k])[:self.config['bottom_n_sectors']]
        
        # 3. Find weakest stocks within those sectors
        candidates = []
        for sym, ret in stock_returns.items():
            ind = industry_map.get(sym, "Unknown")
            if ind in weakest_sectors:
                # Basic trend filter: Close < 50 SMA (must be in a downtrend)
        ...
        # Sort candidates by weakest first
        candidates = sorted(candidates, key=lambda x: x[1])
        
        # Return top N weakest stocks
