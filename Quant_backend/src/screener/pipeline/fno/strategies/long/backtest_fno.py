...
class FnoLongBacktest:
    ...
    def get_futures_data(self, symbols, end_date, lookback_days=600):
        ...
        for sym in symbols:
            ...
            try:
                # Fetch up to 600 daily bars to ensure we have enough lookback
                df = self.tv.fetch_symbol_intraday(fut_sym, interval=Interval.in_daily, n_bars=lookback_days)
    ...
    def run(self):
        logger.info("Initializing F&O Swing Long Backtest...")
        ...
        trading_days = pd.Series(ref_df.index.date).unique()[50:] # Skip first 50 days for lookback
        ...
        for current_date in trading_days:
            ...
            for pos in open_positions:
                ...
                if exit_price > 0:
                    ...
                    stt = exit_price * pos['qty'] * 0.000125 # STT on sell side for futures
