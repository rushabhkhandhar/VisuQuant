"""Canonical E13 Sector Pullback strategy shared by live, forward, and backtest flows."""
...
def generate_e13_signals(
    ...
    """Return the canonical E13 MOC candidate set, ranked and capacity-limited.
    
    Architecture (E13_Sector_Pullback):
      - STATE 1 (Trend, BCR > 0.52):
          Primary:      sector_pullback_eval
    ...
    if bcr > BCR_THRESHOLD:
        state, label = 1, "Trend"
        primary = evaluators["sector_pullback"]
    ...
    for symbol, source_df in bulk_data.items():
        ...
        candidates.append({
            "symbol": symbol,
            "strategy_name": f"E13-{label}-{'Confirmed' if confirmed else 'Primary'}",
            "strategy": f"E13-{label}-{'Confirmed' if confirmed else 'Primary'}",
            "action": "BUY",
