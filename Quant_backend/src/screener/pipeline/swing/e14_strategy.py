"""Canonical E14 Strict AVWAP strategy shared by live, forward, and backtest flows."""
...
def generate_e14_signals(
    ...
    """Return the canonical E14 Strict AVWAP candidate set, ranked and capacity-limited.
    
    Architecture (E14_Strict_AVWAP):
      - STATE 1 (Trend, BCR > 0.52):
          Primary:      avwap_pullback_eval (Strict triple MA alignment + 60d AVWAP test)
          Confirmation: volatility_compression_eval
    ...
    if bcr > BCR_THRESHOLD:
        state, label = 1, "Trend"
        primary = evaluators["avwap_pullback"]
    ...
    for symbol, source_df in bulk_data.items():
        ...
        candidates.append({
            "symbol": symbol,
            "strategy_name": f"E14-{label}-{'Confirmed' if confirmed else 'Primary'}",
            "strategy": f"E14-{label}-{'Confirmed' if confirmed else 'Primary'}",
            "action": "BUY",
