...
3. **Concurrent Dual-Execution**: When you run a strategy, the frontend triggers two parallel engines to prevent selection bias:
   * **The Live Screener (3:15 PM MOC Execution)**: Evaluates all 500 stocks against *live* intraday data via `tvdatafeed` (proxying EOD close) to generate actionable Market-On-Close trades before 3:30 PM.
...
- **`pipeline/compare_strategies.py`**: The core strategy registry. Contains all modular strategy logics (Trend Pullback, Momentum Breakout, Oversold Uptrend, Volatility Compression, Relative Strength) used across the pipeline.
- **`pipeline/run_live_screener.py`**: The dedicated 3:15 PM live execution entry point. It fetches live prices from TradingView, evaluates strategies, and outputs actionable BUY/SELL signals (and Ensemble Candidates) before the market closes.
- **`pipeline/run_front_test.py`**: The EOD Paper Trading Simulator. Runs strategies against historical NSE Bhavcopy data to track portfolio PnL, maintain open trades, and generate strict performance metrics without risking real capital.
- **`pipeline/run_vwap_pullback.py`**: Intraday VWAP Pullback Screener (runs at 12:30 or 1:30 PM). Uses live 15-minute data to find stocks that had a strong morning push but are now resting at their VWAP on extreme volume dry-up.
- **`pipeline/run_btst_hunter.py`**: BTST (Buy Today, Sell Tomorrow) Screener (runs at 3:15 PM). Scans for late-day institutional accumulation to hold overnight for gap-ups.
- **`pipeline/run_intraday_orb.py`**: The 10:15 AM Opening Range Breakout screener. Scans NIFTY 500 for stocks breaking out of their first-hour range with institutional volume, filtered through 6 sequential gates. Outputs ranked candidates with stop-loss/target levels to CSV.
- **`pipeline/run_hikkake_backtest.py`**: A specialized, standalone 2-year backtester evaluating the Inside Bar Fakeout (Hikkake) pattern over 500 historical trading days.
...
To run the **Live 3:15 PM Strategy Execution**, run:
```bash
cd Quant_backend
